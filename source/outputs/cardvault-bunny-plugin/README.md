...
Bunny / Vendetta 插件，版本 1.1.3。将 Discord 的 PNG 和 JSON 附件保存到 CardVault。

## 1.1.3 简体中文套装

插件名称及介绍改为简体中文，上传行为沿用 1.1.2。构建时同时生成独立的长按选字插件。
使用 Bunny 277 套装时，以套装根目录的 `README-Bunny-277.md` 为准；其中使用现有 `card2new` 仓库及两个独立插件目录，不需要新仓库。
